...
enum {
	...
	roundedDevice				= 5,							/* 1 if device has been \322rounded\323 into the GrayRgn */
...
// NOTE: Name conflict!!!
struct mycarbonCursor {
	Bits16							data;
	Bits16							mask;
	Point							hotSpot;
};
typedef struct mycarbonCursor mycarbonCursor;

typedef mycarbonCursor *CursPtr, **CursHandle;
...
// NOTE: Name conflict!
struct mycarbonRegion {
	short							rgnSize;					/*size in bytes*/
	Rect							rgnBBox;					/*enclosing rectangle*/
};
typedef struct mycarbonRegion mycarbonRegion;

typedef mycarbonRegion *RgnPtr, **RgnHandle;
...
struct GrafPort {
    ...
    /*
     * Add filler to align GrafPort.pnVis with CGrafPort.pnVis
     */
    short filler1;
#endif
	Pattern							pnPat;
...
/*
 * Quickdraw globals are really global.
 * We fake out the structure references with a class of inline defs.
 */

extern long fontAdj;
extern Point patAlign;
extern short polyMax;
extern PolyHandle thePoly;
extern short playIndex;
extern PicHandle playPic;
extern short rgnMax;
extern short rgnIndex;
extern Handle rgnBuf;
extern mycarbonRegion wideData;
extern RgnPtr wideMaster;
extern RgnHandle wideOpen;
extern long randSeed;
extern BitMap screenBits;
extern mycarbonCursor arrow;
...
typedef _qd QDGlobals, *QDGlobalsPtr, **QDGlobalsHdl;
...
extern pascal void SetCursor(const mycarbonCursor *crsr)
